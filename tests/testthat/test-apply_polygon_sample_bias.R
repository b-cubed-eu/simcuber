...
test_that("arguments are of the right class", {
  ...
  expect_error(apply_polygon_sample_bias(data.frame(x = 1, y = 1),
               ...
               fixed = TRUE)
  expect_error(apply_polygon_sample_bias(occurrences_sf = 1,
                                         bias_area,
                                         bias_strength),
               regexp = "`occurrences_sf` must be an sf object",
               fixed = TRUE)
  expect_error(apply_polygon_sample_bias(occurrences_sf = "string",
                                         bias_area,
                                         bias_strength),
               regexp = "`occurrences_sf` must be an sf object",
               fixed = TRUE)
  ...
  expect_error(apply_polygon_sample_bias(occurrences_sf,
                                         data.frame(x = 1, y = 1),
                                         bias_strength = 1),
...
test_that("bias_weight column contains values between 0 and 1", {
  ...
  result <- apply_polygon_sample_bias(occurrences_sf,
                                      bias_area,
