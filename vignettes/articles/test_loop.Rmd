...
```{r}
n_sim <- 5
out_list <- vector("list", length = n_sim)
...
for (i in seq_len(n_sim)) {
  ...
  occurrence_cube_df$sim <- i
  
  out_list[[i]] <- occurrence_cube_df
}
occurrence_sims_df <- do.call(rbind.data.frame, out_list)
```
...
points are designated to grid, buffer is visible
